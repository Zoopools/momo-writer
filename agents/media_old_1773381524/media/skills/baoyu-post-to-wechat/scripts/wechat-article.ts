...
async function pasteFromClipboardInEditor(session: ChromeSession, retryCount: number = 0): Promise<void> {
  ...
  if (!editorHasContent && retryCount < 2) {
    console.warn(`[wechat] Paste verification failed, retrying (${retryCount + 1}/2)...`);
    await sleep(500);
    await pasteFromClipboardInEditor(session, retryCount + 1);
  } else if (!editorHasContent) {
    console.error('[wechat] Paste failed after 3 attempts. Editor still empty.');
  } else {
    console.log('[wechat] Paste verified OK.');
  }
}
